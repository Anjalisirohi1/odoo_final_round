...
class UnifiedDealContext:
    ...
    def product_ids(self) -> List[str]:
        """Extracts unique product IDs from quotation items or quotation."""
        pids = []
        for item in self.items:
            pid = item.get("product_id")
            if pid and pid not in pids:
                pids.append(pid)
        # Fallback to quotation product_ids if items are empty
        if not pids and "product_ids" in self.quotation:
            pids = list(self.quotation["product_ids"])
        return pids

    @property
    def customer_id(self) -> Optional[str]:
        return self.quotation.get("customer_id")

    @property
    def total_amount(self) -> float:
        return float(self.quotation.get("total_amount", 0.0))

    @property
    def discount_percentage(self) -> float:
        return float(self.quotation.get("discount_percentage", 0.0))


class UnifiedContextBuilder:
    """
    Constructs a UnifiedDealContext for a requested quotation from provided datasets.
    """
    def __init__(
        self,
    ...
    def build_context(self, quotation_id: str, custom_now: Optional[datetime] = None) -> Optional[UnifiedDealContext]:
        """
        Builds and returns UnifiedDealContext for quotation_id. Returns None if quotation not found.
        """
        ...
        items = self.items_by_quote.get(quotation_id, [])
